...
static int _create_model(void)
{
	...
	for (example_index = 0; example_index < 10; ++example_index) {
	   ...
	   snprintf(filePath, FILEPATH_SIZE, "%simages/face_sample_%d.JPG", app_res_dir, example_index);
...
static int _recognize_face()
{
	...
	snprintf(filePath, FILEPATH_SIZE, "%s/images/face_sample_5.JPG", app_res_dir);
	free(app_res_dir);
...
static const char *_colorspace_int_to_str(image_util_colorspace_e colorspace)
{
	char *str = NULL;
	switch (colorspace) {
...
{
	_D("%s", _colorspace_int_to_str(colorspace));
	return true;
}
